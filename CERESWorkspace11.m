...
ax = worldmap('World');setm(ax, 'Origin', [0 180 0]);
    curPlot = MovingAvg.Window12Months.SW(:,:,k);
    geoshow(curPlot,geoidrefvec,'DisplayType','texturemap');colorbar
    load coast;plotm(lat,long,'black')
%     MaxBeta = max(max(abs(curPlot)));
%     caxis([-MaxBeta MaxBeta])
...
set(gca,'FontSize',20)
ax = worldmap('World');setm(ax, 'Origin', [0 180 0]);
    curPlot = MovingAvg.Window12Months.LW(:,:,k);
    geoshow(curPlot,geoidrefvec,'DisplayType','texturemap');colorbar
    load coast;plotm(lat,long,'black')
%     MaxBeta = max(max(abs(curPlot)));
%     caxis([-MaxBeta MaxBeta])
...
set(gca,'FontSize',20)
ax = worldmap('World');setm(ax, 'Origin', [0 180 0]);
    curPlot = MovingAvg.Window12Months.Temp(:,:,k);
    geoshow(curPlot,geoidrefvec,'DisplayType','texturemap');colorbar
    load coast;plotm(lat,long,'black')
    MaxBeta = max(max(abs(curPlot)));
     caxis([-4 4])
...
print(gcf,'-dpng','-r300',['NetFrame',num2str(k),'.png']);
