function [FluxDeparture] = GlobalValuesMinusClimatology(Flux,VariableName)
...
FluxDeparture = zeros(lats,longs,time);
%Flux = LWclear;
...
geoshow(resizem(std(FluxDeparture,0,3),Factor),geoidrefvec,'DisplayType','texturemap');colorbar
...
ax = worldmap('World');setm(ax, 'Origin', [0 180 0]);land = shaperead('landareas', 'UseGeoCoords', true);geoshow(ax, land, 'FaceColor', [0.5 0.7 0.5])
geoshow(resizem(mean(Flux,3),Factor),geoidrefvec,'DisplayType','texturemap');colorbar
...
grid on;
set(gca,'xtick',10/Factor:10/Factor:180/Factor)
