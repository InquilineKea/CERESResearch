...
allMonthWeights = repmat(days_per_month,1,time/12-1); %number of years
...
geoshow(mean(MovingAverage,3),geoidrefvec,'DisplayType','texturemap');h=colorbar
...
caxis([-max(max(abs(squeeze(mean(MovingAverage,3))))) max(max(abs(squeeze(mean(MovingAverage,3)))))])
...
colormap('default')
ax = worldmap('World');setm(ax, 'Origin', [0 180 0]);land = shaperead('landareas', 'UseGeoCoords', true);geoshow(ax, land, 'FaceColor', [0.5 0.7 0.5])
geoshow(std(MovingAverage,0,3),geoidrefvec,'DisplayType','texturemap');h=colorbar %flux departure; doesnt matter here
view(2)
set(gca,'FontSize',20);set(h,'FontSize',20)
% title(['Temporal ',num2str(MonthFilterSize),' Month Moving Average SD of ',VariableName])
title([VariableName, ' Standard Deviation (W/m^2)'])
set(gca,'GridLineStyle','--')
set(gcf,'paperposition',[0 0 20 10])
...
% print(gcf,'-dpng','-r300',[VariableName, num2str(MonthFilterSize),'Month_MA_Latitudinal_SD','.png']);
 print(gcf,'-dpng','-r300',[VariableName, num2str(MonthFilterSize),'TotalSD','.png']);
...
ylabel('Latitude')
set(gca,'xtick',[12-2-(MonthFilterSize-1)-6:12:time]) %what if Filter Size is 12 months?
...
if strcmp(VariableName,'Cloud')
   title([VariableName, ' Radiative Forcing Anomaly (W/m^2)']) 
elseif strcmp(VariableName,'Precip')
    title([VariableName, ' Anomaly (mm/day)']) 
else
    title([VariableName, ' Radiation Anomaly (W/m^2)'])
end
set(gca,'GridLineStyle','--')
set(gcf,'paperposition',[0 0 20 10])
